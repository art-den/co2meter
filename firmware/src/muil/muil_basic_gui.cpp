...
class CharactersPrinter : public ICharactersConsumer
{
public:
	CharactersPrinter(
		int            x,
	...
	void consume(wchar_t character) override
	{
		if (character == ' ')
		{
			int space_width = font_.heightPages/4;
...
class TextWidthCalculator : public ICharactersConsumer
{
public:
...
void display_print_any(
	int                       x,
	int                       y,
	const ICharactersProvider &provider,
	const FontInfo            &font,
	const Color               &color,
	const Color               *bg_color)
{
...
void display_print_any_in_rect(
	const Rect                &rect,
	HorizAlign                align,
	const ICharactersProvider &provider,
	const FontInfo            &font,
	...
	const Color               *bg_color)
{
	...
	display_print_any(x1, y1, provider, font, color, bg_color);
	...
	const Color    *bg_color)
{
	...
	display_print_any(x, y, provider, font, color, bg_color);
}
...
{
	Utf16CharactersProvider provider(text);
	display_print_any_in_rect(rect, align, provider, font, color, bg_color);
}
